...
export default {
  ...
  devicesManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.title);
  },
  devicesOneListFormatter(val) {
    if (!val) return '';
    return val.title;
  },
  devicesManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.title };
    });
  },
  devicesOneListFormatterEdit(val) {
    if (!val) return '';
    return { label: val.title, id: val.id };
  },

  studiosManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.name);
  },
  studiosOneListFormatter(val) {
    if (!val) return '';
    return val.name;
  },
  studiosManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.name };
    });
  },
  studiosOneListFormatterEdit(val) {
    if (!val) return '';
