...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/studios/studiosSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureStudiosCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import CardStudios from './CardStudios';

const perPage = 10;

const TableSampleStudios = ({ filterItems, setFilterItems, filters }) => {
  ...
  const {
    ...
    loading,
    count,
    notify: studiosNotify,
    refetch,
  } = useAppSelector((state) => state.studios);
  ...
  useEffect(() => {
    if (studiosNotify.showNotification) {
      notify(studiosNotify.typeNotification, studiosNotify.textNotification);
    }
  }, [studiosNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/studios/studios-view/?id=${id}`);
  };
...
export default TableSampleStudios;
