...
const CardUsers = ({
  ...
  const hasUpdatePermission = hasPermission(currentUser, 'UPDATE_DEVICES');
          ...
          devices.map((item, index) => (
            ...
            >
              ...
              <dl className='divide-y divide-gray-100 dark:divide-dark-700 px-6 py-4 text-sm leading-6 h-64 overflow-y-auto'>
                ...
                <div className='flex justify-between gap-x-4 py-3'>
                  <dt className='text-gray-500 dark:text-dark-600'>Studio</dt>
                  <dd className='flex items-start gap-x-2'>
                    <div className='font-medium line-clamp-4'>
                      {dataFormatter.studiosOneListFormatter(item.studio)}
                    </div>
                  </dd>
                </div>
